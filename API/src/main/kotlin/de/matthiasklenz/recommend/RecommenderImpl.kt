...
class RecommenderImpl : KoinComponent, Recommender {
    ...
    override fun recommendUserBasedItemFor(
        ...
        if (ratings.find { it.userid == userid } == null) {
            logger.error(
                "User $userid is not within the list provided! Returning empty map!"
            )
            return mapOf()
        }
        val returnRecommendations =
            mutableMapOf<String, Double>()
        val similarities =
            getUserSimilaritiesOf(
                userid,
                similarityMeasure,
                allItems,
                ratings
        ...
        unratedItems.forEach { item ->
            ...
            val neighbors = similarities.filter {
                it.userid != userid && it.ratings.containsKey(
                    item
                )
            }
    ...
    override fun getUserSimilaritiesOf(
        ...
        if (ratings.find { it.userid == userid } == null) {
            logger.error(
                "User $userid is not within the list provided! Returning empty list!"
            )
            return listOf()
        }
    ...
    override fun getItemSimilaritiesOf(
        ...
        if (!allItems.contains(item)) {
            logger.error(
                "Item $item is not within the list provided! Returning empty list!"
            )
            return listOf()
        }
